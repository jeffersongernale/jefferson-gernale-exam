...
class youtubeChannel {
   ...
   function getYtVideosAPI($channel_id){
     ...
     for($x = 0; $x < 2; $x++)
     {
          ...
          $curl = curl_init();
          curl_setopt($curl, CURLOPT_URL, "https://youtube.googleapis.com/youtube/v3/search?channelId={$channel_id}&key={$this->api_key}&&part=snippet&order=date&maxResults=50&type=video{$appendString}");
          curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
          $result = curl_exec($curl);
          curl_close($curl);
